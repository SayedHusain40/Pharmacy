...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    // Update data in 'user data' table
    $sql = $db->prepare("UPDATE `user data` SET Password = ?, Type = ?, Email = ? WHERE Username = ?");
    $sql->execute([$hps, $user_type, $email, $user]);

    // Retrieve the user_data_id
    $sql = $db->prepare("SELECT UserID FROM `user data` WHERE Username = ?");
    $sql->execute([$user]);
    $user_data_id = $sql->fetchColumn();

    if ($user_type == "Customer") {
        // Update data in 'customer data' table
        $sql = $db->prepare("UPDATE `customer data` SET UserID = ?, FirstName = ?, LastName = ?, MobileNumber = ?, Email = ?, Area = ?, House = ?, Street = ?, Block = ?, Gender = ?, DOB = ? WHERE CustomerID = ?");
        $sql->execute([$user_data_id, $fname, $lname, $phone, $email, $Area, $House, $Street, $Block, $Gender, $DOB, $customerId]);
    } else if ($user_type == "Staff") {
        // Update data in 'Staff data' table
        $sql = $db->prepare("UPDATE `Staff data` SET UserID = ?, FirstName = ?, LastName = ?, MobileNumber = ?, Email = ?, Area = ?, House = ?, Street = ?, Block = ?, Gender = ?, DOB = ?, CPR = ?, AcademicDegree = ?, StaffPosition = ? WHERE StaffID = ?");
        $sql->execute([$user_data_id, $fname, $lname, $phone, $email, $Area, $House, $Street, $Block, $Gender, $DOB, $CPR, $ADegree, $SPosition, $staffId]);
    } else if ($user_type == "Supplier") {
        // Update data in 'Supplier data' table
        $sql = $db->prepare("UPDATE `Supplier data` SET UserID = ?, FirstName = ?, LastName = ?, MobileNumber = ?, Email = ? WHERE SupplierID = ?");
        $sql->execute([$user_data_id, $fname, $lname, $phone, $email, $supplierId]);
    }
if ($sql->rowCount() === 1) {
    $_SESSION['Edit User'] = true;
...
<?php endif; ?>
      <fieldset class="fieldset-xl">
        ...
        <select name="user_type" id="user_type" title="Select the user type you want to edit!">
...
if (isset($_POST['user_type'])) {
    ...
    if ($user_type === 'Staff' || $user_type === 'Customer' || $user_type === 'Supplier') {
      ...
      echo '<label for="user_type">' . $user_type  . '</label>';
      echo '<select name="user_type" id="user_type" title="Select the user you want to edit!">';
...
?>
    ...
    <div class="form-group">
      ...
      <input type="text" class="form-control" id="Block" name="Block" placeholder="Enter block" title="Enter block" value="<?php echo $Block; ?>">
          ...
          <input type="submit" name="Edit User" value="Edit User"/>
          </div>
